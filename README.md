...
2. **`find_threshold.py`** is a tkinter app for interactively tuning the Gaussian blur and percentile threshold until the mask for the embryo is satisfactory. It saves the parameters to a YAML file. It accepts both ND2 files with multiple embryos and TIF files with just one embryo.
...
- **`movie_from_nd2.py`** generates a quick unaligned MP4 preview straight from the ND2 file.
