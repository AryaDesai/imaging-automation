...
Tools for Ozbudak lab that process Nikon ND2 files from multi-embryo timelapse experiments. Corrects XY and Z drift across timepoints so each embryo stays centered throughout the movie, then exports OME-TIFFs for downstream analysis and MP4s for inspection.
...
2. **`find_threshold.py`** is a tkinter app for interactively tuning the Gaussian blur and threshold until the embryo mask is satisfactory. It saves the parameters to a YAML config file. It accepts both ND2 files with multiple embryos and single-embryo TIF files.
...
4. **`align_z.py`** corrects focal-plane drift by tracking the Z intensity centroid of the threshold channel across time.
...
```bash
conda create -n imaging-automation -c conda-forge python=3.11 nd2 numpy scipy tifffile imageio pillow matplotlib tqdm pyyaml pytorch
```
